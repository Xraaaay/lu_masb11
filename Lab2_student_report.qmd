...
As defined in the Model Selection section, we assume that the waiting time in the morning follows a log-normal distribution: $$y_{am} \sim LogNormal(\mu_{am}, \sigma_{am})$$
...
As defined in the Model Selection section, we assume that the waiting time in the afternoon also follows a log-normal distribution: $$y_{pm} \sim LogNormal(\mu_{pm}, \sigma_{pm})$$
...
We use the Law of Total Probability to calculate the probability of all-day waiting time exceeding two hours: $$P(T > 120) = P(T > 120 \vert AM) \cdot P(AM) + P(T > 120 \vert PM) \cdot P(PM)$$
...
**Answer**: The probability that a patient coming any time waits more than two hours is `r sprintf("%.2f", prob_gt_120 * 100)`%.
